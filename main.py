...
class ModelInference:
    ...
    def _initialize_special_tokens(self) -> None:
        ...
        special_tokens_dict = {
            k: v for k, v in {
                ...
                "bos_token": '<s>',
                "unk_token": '<unk>'
            }.items() if getattr(self.tokenizer, k) is None
        }
        if special_tokens_dict:
            self.tokenizer.add_special_tokens(special_tokens_dict)
            self.tokenizer.save_pretrained(self.model_path)

    def generate_outputs(self, dataset: torch.utils.data.Dataset) -> Tuple[List[str], List[str]]:
        """
    ...
    def extract_embeddings(self, outputs: List[str], prompts: List[str], embedding_type: str = "Average") -> Tuple[List[List[float]], List[List[float]]]:
        """
        ...
        for output, prompt in tqdm(zip(outputs, prompts), desc="Extracting embeddings"):
            with torch.no_grad():
                input_ids = self.tokenizer(prompt, return_tensors="pt").input_ids.to(self.device)
                output_ids = self.tokenizer(output, return_tensors="pt").input_ids.to(self.device)
    ...
    def _compute_embedding(self, token_ids: torch.Tensor, embedding_type: str) -> List[float]:
        """
        ...
        """
        model_output = self.model(token_ids, return_dict=True, output_hidden_states=True)
...
def main(args: argparse.Namespace) -> None:
    ...
    input_embeddings, output_embeddings = model_inference.extract_embeddings(outputs, prompts, embedding_type="Average")
    model_inference.save_embeddings(input_embeddings, os.path.join(args.output_dir, "input_embeddings.csv"))
    model_inference.save_embeddings(output_embeddings, os.path.join(args.output_dir, "output_embeddings.csv"))

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--model-path", type=str, required=True, help="Path to model weights.")
    parser.add_argument("--model-id", type=str, required=True)
    parser.add_argument("--data-file", type=str, required=True, help="Path to the dataset file.")
    parser.add_argument("--dataset", type=str, required=True, choices=['smoking', 'cancer'], help="Dataset type.")
    parser.add_argument("--output-dir", type=str, required=True, help="Directory to save outputs.")
    parser.add_argument("--max-new-token", type=int, default=1024, help="Max tokens generated.")
