...
def DataClean(data, labels_path, data_path=None, delete=None):
    ...
    ints_str = (
        "0123456789-#[]"  # characters that usually main categories don't start with
    )
    ...
    for note_index in tqdm(range(notes.shape[0])):
        ...
        for p in paragraphs:
            ...
            if (
                len(line) > 0
                and ":" in line
                and not (line[line.find(":") - 1] in ints_str)
                and not (line[0] in ints_str)
            ):
        ...
        note_df = pd.DataFrame(
            {"title": note_sect_tit, "category": "", "text": note_sect_par, "label": ""}
        )
        ...
        for i in range(len(notes_sections[key]["category"])):
            if buffer != notes_sections[key]["category"][i]:
                ...
                note_sect_par[-1] = (
                    note_sect_par[-1]
                    + " "
                    + notes_sections[key]["title"][i]
                    + " "
                    + notes_sections[key]["text"][i]
                )
        note_df = pd.DataFrame(
            {"title": note_sect_tit, "text": note_sect_par, "label": note_sect_lab}
        )
    ...
    if delete != None:
        for key, value in notes_sections.items():
            notes_sections[key] = notes_sections[key][
                ~notes_sections[key]["label"].isin(delete)
            ]
