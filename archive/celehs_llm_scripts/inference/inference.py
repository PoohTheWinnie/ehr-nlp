...
messages = [
    [
        (
            "Human",
            few_shot_question_template.format(
                context=item["context"], question=item["question"]
            ),
        ),
        ("Assistant", item["answer"]),
    ]
    for item in incontext_few_shots
]
...
def run_eval(
    ...
    try:
        ...
        model = LLM(
            model=model_path, tokenizer_mode="slow", tensor_parallel_size=tp_size
        )
    ...
    for item in tqdm(questions):
        ...
        qs = few_shot_question_template.format(
            context=item["context"], question=item["question"]
        )
    ...
    for output in outputs:
        ...
        model_output = raw_model(
            output_tokens, return_dict=True, output_hidden_states=True
        )
...
def run_eval_extract_embeddings(
    ...
    embeddings.to_csv(
        os.path.dirname(answer_file) + "/embeddings.csv",
        sep="\t",
        encoding="utf-8",
        index=False,
    )
...
if __name__ == "__main__":
    ...
    if "WORLD_SIZE" in os.environ and int(os.environ["WORLD_SIZE"]) > 1:
        ...
        devices = ",".join(
            [str(i) for i in range(rank * tp_size, (rank + 1) * tp_size)]
        )
